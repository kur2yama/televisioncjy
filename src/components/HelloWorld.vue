<template>
  <div class="hello">
    ...
    <ul v-if="playtoggle=='video'">
      <li v-for="(item, i) in videoList" :key="i" :class="{'playing':playid==item.id}" @click="switchSource(item)">{{item.name}}</li>
    </ul>
    <ul v-if="playtoggle=='audio'">
      <li v-for="(item, i) in audioList" :key="i" :class="{'playing':playid==item.id}" @click="switchSource(item)">{{item.name}}</li>
    </ul>
  </div>
</template>
...
export default {
  ...
  mounted() {
    var _this = this;
    getAuth({origin_stream:'http://live21-cjy.hbtv.com.cn/hbtv/hbtv-hbws.m3u8'}).then(res => {
          if (res.state) {
...
ul li {
  ...
  float: left;
  width: 50%;
  background: url("http://hudong.hbtv.com.cn/uploads/cjyuncalendar/bg_corner_gray.png")
    no-repeat 100% 100%;
}
